...
void UIDrawList::PushClipRect(float x, float y, float width, float height) {
    ...
    if (!m_clipRectStack.empty() && m_clipRectStack.back().Enabled) {
        const UIClipRect& current = m_clipRectStack.back();
        const float left = (std::max)(current.X, clipRect.X);
        const float top = (std::max)(current.Y, clipRect.Y);
        const float right = (std::min)(current.X + current.Width, clipRect.X + clipRect.Width);
        const float bottom = (std::min)(current.Y + current.Height, clipRect.Y + clipRect.Height);
        clipRect.X = left;
        clipRect.Y = top;
