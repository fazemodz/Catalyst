...
bool ScriptModuleHost::PerformPendingReload(std::wstring* outError) {
    ...
    const bool isFirstLoad = (m_loadedWriteTimeTicks == 0);
    ...
    const std::string dllName = fs::path(m_sourceDllPath).filename().string();
    if (isFirstLoad) {
        OutputLog::Add("Script module loaded: " + dllName);
    } else {
        OutputLog::Add("Script module reloaded: " + dllName);
    }
    return true;
}
