...
namespace Catalyst {
constexpr uint32_t kScriptApiVersion = 4;
...
struct ScriptHostApi {
    ...
    void (*logWarning)(void* userData, const char* message) = nullptr;
    void (*logError)(void* userData, const char* message) = nullptr;
...
protected:
    ...
    void LogWarning(const char* message) const {
        if (m_hostApi != nullptr && m_hostApi->logWarning != nullptr) {
            m_hostApi->logWarning(m_hostApi->userData, message != nullptr ? message : "");
        }
    }

    void LogError(const char* message) const {
        if (m_hostApi != nullptr && m_hostApi->logError != nullptr) {
            m_hostApi->logError(m_hostApi->userData, message != nullptr ? message : "");
        }
    }
