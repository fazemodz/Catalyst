...
void EngineApp::Run(HINSTANCE hInstance, int nShowCmd) {
    ...
    const bool hasLaunchRequest = ConsumeAppLaunchRequest(startupRequest);
    const bool standalonePlayLaunch = hasLaunchRequest && startupRequest.play;
    const bool projectEditorLaunch = hasLaunchRequest && !startupRequest.play && !startupRequest.projectFilePath.empty();
    if (standalonePlayLaunch || projectEditorLaunch) {
        SetAppLaunchRequest(startupRequest);
    }

    DWORD launcherStyle = (standalonePlayLaunch || projectEditorLaunch)
        ? WS_OVERLAPPEDWINDOW
        : (WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX);

    int screenW = GetSystemMetrics(SM_CXSCREEN);
    int screenH = GetSystemMetrics(SM_CYSCREEN);
    int launchW = (standalonePlayLaunch || projectEditorLaunch) ? screenW : 1000;
    int launchH = (standalonePlayLaunch || projectEditorLaunch) ? screenH : 650;

    WindowContext* mainWindow = CreateWindowContext(
        (standalonePlayLaunch || projectEditorLaunch) ? BuildStandalonePlayTitle(startupRequest) : L"Catalyst Launcher",
        launcherStyle,
        launchW,
        launchH,
        (standalonePlayLaunch || projectEditorLaunch) ? 0 : (screenW - launchW) / 2,
        (standalonePlayLaunch || projectEditorLaunch) ? 0 : (screenH - launchH) / 2);
    if (!mainWindow) {
    ...
    ShowWindow(mainWindow->hwnd, (standalonePlayLaunch || projectEditorLaunch) ? SW_MAXIMIZE : nShowCmd);
