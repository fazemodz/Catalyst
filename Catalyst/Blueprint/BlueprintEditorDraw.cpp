...
BlueprintEditor::DrawLayout BlueprintEditor::ComputeLayout(float width, float height) const {
    DrawLayout L{};
    L.width = width;
    L.height = height;
    L.menuBarH = 28.0f;
    ...
    L.graphTabH = 32.0f;
    L.graphToolsH = 34.0f;
    L.workspaceTop = L.menuBarH + L.toolBarH + L.graphTabH + L.graphToolsH;
    L.paletteW = std::clamp(width * 0.17f, 316.0f, 356.0f);
    L.inspectorW = std::clamp(width * 0.22f, 384.0f, 440.0f);
    L.canvasX = L.paletteW;
    L.canvasY = L.workspaceTop;
    L.canvasW = (std::max)(280.0f, width - L.paletteW - L.inspectorW);
    L.canvasH = (std::max)(180.0f, height - L.workspaceTop);
    L.inspectorX = width - L.inspectorW;
    L.contentBottom = height;
    L.toolY = L.menuBarH + 5.0f;
    L.toolTextY = L.toolY + 15.0f;
    L.assetInfoW = std::clamp(width * 0.18f, 180.0f, 280.0f);
    L.assetInfoX = width - L.assetInfoW - 18.0f;
    L.graphToolsY = L.menuBarH + L.toolBarH + L.graphTabH;
    L.visibleCenterGraphX = (L.canvasW * 0.5f - m_pan.x) / m_zoom;
    L.visibleCenterGraphY = (L.canvasH * 0.5f - m_pan.y) / m_zoom;
    L.isViewportTab = m_activeTab == DocumentTab::Viewport;
    L.isConstructionTab = m_activeTab == DocumentTab::ConstructionScript;
    L.isEventGraphTab = m_activeTab == DocumentTab::EventGraph;
    L.isUIAsset = IsUIAsset();
    L.now = GetTickCount();
    return L;
}

void BlueprintEditor::DrawMenuToolbar(UIDrawList& drawList, UIContext& uiCtx, FontManager& fontManager, InputManager& inputManager,
                                      const DrawLayout& L, const GameObject* selectedObject) {
    const float canvasX = L.canvasX;
    const float canvasY = L.canvasY;
    const float canvasW = L.canvasW;
    const float canvasH = L.canvasH;
    const float menuBarH = L.menuBarH;
    const float toolBarH = L.toolBarH;
    const float graphTabH = L.graphTabH;
    const float graphToolsH = L.graphToolsH;
    const float workspaceTop = L.workspaceTop;
    const float toolY = L.toolY;
    const float toolTextY = L.toolTextY;
    const float assetInfoW = L.assetInfoW;
    const float assetInfoX = L.assetInfoX;
    const float graphToolsY = L.graphToolsY;
    const float visibleCenterGraphX = L.visibleCenterGraphX;
    const float visibleCenterGraphY = L.visibleCenterGraphY;
    const bool isViewportTab = L.isViewportTab;
    const bool isConstructionTab = L.isConstructionTab;
    const bool isEventGraphTab = L.isEventGraphTab;
    const bool isUIAsset = L.isUIAsset;
    const DWORD now = L.now;
    const bool interactionsBlocked = L.interactionsBlocked;
    const HWND windowHandle = L.windowHandle;
...
void BlueprintEditor::DrawPalettePanel(UIDrawList& drawList, UIContext& uiCtx, FontManager& fontManager, InputManager& inputManager,
                                       const DrawLayout& L) {
    const float canvasX = L.canvasX;
    const float canvasY = L.canvasY;
    const float canvasW = L.canvasW;
    const float canvasH = L.canvasH;
    const float workspaceTop = L.workspaceTop;
    const float contentBottom = L.contentBottom;
    const float graphToolsY = L.graphToolsY;
    const float visibleCenterGraphX = L.visibleCenterGraphX;
    const float visibleCenterGraphY = L.visibleCenterGraphY;
    const bool isViewportTab = L.isViewportTab;
    ...
    const bool isUIAsset = L.isUIAsset;
    const bool interactionsBlocked = L.interactionsBlocked;
    const float paletteW = L.paletteW;
...
void BlueprintEditor::DrawInspectorPanel(UIDrawList& drawList, UIContext& uiCtx, FontManager& fontManager, InputManager& inputManager,
                                         const DrawLayout& L, const GameObject* selectedObject) {
    const float canvasX = L.canvasX;
    const float canvasY = L.canvasY;
    const float canvasW = L.canvasW;
    ...
    const float workspaceTop = L.workspaceTop;
    const float contentBottom = L.contentBottom;
    const float inspectorX = L.inspectorX;
    const float inspectorW = L.inspectorW;
    const bool isViewportTab = L.isViewportTab;
    const bool isConstructionTab = L.isConstructionTab;
    const bool isEventGraphTab = L.isEventGraphTab;
    const bool isUIAsset = L.isUIAsset;
...
void BlueprintEditor::DrawViewportTab(UIDrawList& drawList, UIContext& uiCtx, FontManager& fontManager, InputManager& inputManager,
                                      const DrawLayout& L) {
    const float canvasX = L.canvasX;
    const float canvasY = L.canvasY;
    const float canvasW = L.canvasW;
...
void BlueprintEditor::DrawConstructionTab(UIDrawList& drawList, UIContext& uiCtx, FontManager& fontManager, InputManager& inputManager,
                                          const DrawLayout& L) {
    const float canvasX = L.canvasX;
    const float canvasY = L.canvasY;
    const float canvasW = L.canvasW;
...
void BlueprintEditor::DrawEventGraphTab(UIDrawList& drawList, UIContext& uiCtx, FontManager& fontManager, InputManager& inputManager,
                                        const DrawLayout& L, const GameObject* selectedObject) {
    const float canvasX = L.canvasX;
    const float canvasY = L.canvasY;
    const float canvasW = L.canvasW;
