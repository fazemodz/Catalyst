...
void QuantaMeshPass::CreateComputeBuffers(ID3D12Device* device)
{
    ...
    for (UINT frame = 0; frame < kFrameCount; ++frame) {
        ...
        ThrowIfFailed(device->CreateCommittedResource(&uploadHeap, D3D12_HEAP_FLAG_NONE, &bufferDesc, D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&m_objectDataBuffer[frame])));
        m_objectDataBuffer[frame]->Map(0, nullptr, (void**)&m_mappedObjectData[frame]);

        bufferDesc.Width = (sizeof(GlobalBufferData) + 255) & ~255;
        ThrowIfFailed(device->CreateCommittedResource(&uploadHeap, D3D12_HEAP_FLAG_NONE, &bufferDesc, D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&m_globalCB[frame])));
        m_globalCB[frame]->Map(0, nullptr, (void**)&m_mappedGlobalCB[frame]);
        ...
        ThrowIfFailed(device->CreateCommittedResource(&defaultHeap, D3D12_HEAP_FLAG_NONE, &bufferDesc, D3D12_RESOURCE_STATE_COMMON, nullptr, IID_PPV_ARGS(&m_counterBuffer[frame])));
    }
    ...
    uint32_t* zero = nullptr;
    ThrowIfFailed(m_zeroBuffer->Map(0, nullptr, (void**)&zero));
    *zero = 0;
...
void QuantaMeshPass::CreatePipelines(ID3D12Device* device) {
    ...
    // Variant used by the main scene pass: it renders into the HDR target and
    // additionally fills the raytracing G-buffer.
    D3D12_GRAPHICS_PIPELINE_STATE_DESC psoGBuffer = psoQ;
    psoGBuffer.NumRenderTargets = 3;
    psoGBuffer.RTVFormats[0] = DXGI_FORMAT_R16G16B16A16_FLOAT;
    psoGBuffer.RTVFormats[1] = DXGI_FORMAT_R8G8B8A8_UNORM;
    psoGBuffer.RTVFormats[2] = DXGI_FORMAT_R32G32B32A32_FLOAT;
    // RenderTarget[1..2] are zero-initialised, which means a write mask of 0 -
    // the G-buffer would be silently masked off. Match target 0.
    psoGBuffer.BlendState.RenderTarget[1] = psoGBuffer.BlendState.RenderTarget[0];
    psoGBuffer.BlendState.RenderTarget[2] = psoGBuffer.BlendState.RenderTarget[0];
    const HRESULT gbufferPsoResult =
        device->CreateGraphicsPipelineState(&psoGBuffer, IID_PPV_ARGS(&m_quantaPipelineStateGBuffer));
    if (FAILED(gbufferPsoResult)) {
        m_quantaPipelineStateGBuffer.Reset(); // fall back to the colour-only pipeline
    }
...
void QuantaMeshPass::Render(ID3D12GraphicsCommandList* commandList, ID3D12Device* device, const std::vector<GameObject>& gameObjects, const Camera& camera, int width, int height, DirectX::XMMATRIX lightSpaceMatrix, DirectX::XMFLOAT3 activeLightDir, float activeIntensity, Texture* activeSkybox, ID3D12DescriptorHeap* bindlessHeap, UINT srvDescriptorSize, UINT frameIndex, bool writeGBuffer, Texture* texWhite, Texture* texNormal, Texture* texBlack, Mesh* defaultSphereMesh)
{
    ...
    ID3D12PipelineState* meshPipeline =
    ...
    memcpy(m_mappedMaterialCB[frameIndex], &mat, sizeof(MaterialConstants));
    ...
    for (auto& pair : meshGroups) {
        ...
        for (size_t i = 0; i < instances.size(); i++) {
            ...
            m_mappedObjectData[frameIndex][index].worldMatrix = XMMatrixTranspose(scale * rot * trans);
            ...
            m_mappedObjectData[frameIndex][index].normalIndex = normalTexture ? normalTexture->GetBindlessIndex() : texNormal->GetBindlessIndex();
            m_mappedObjectData[frameIndex][index].metallicIndex = metallicTexture ? metallicTexture->GetBindlessIndex() : texBlack->GetBindlessIndex();
            m_mappedObjectData[frameIndex][index].roughnessIndex = roughnessTexture ? roughnessTexture->GetBindlessIndex() : texWhite->GetBindlessIndex();
            m_mappedObjectData[frameIndex][index].roughnessScale = obj->roughness;
            const bool showMeshlets = obj->asset != nullptr && obj->asset->useVirtualGeometry && obj->asset->debugVisualizer;
            m_mappedObjectData[frameIndex][index].debugMeshletSize = showMeshlets ? kMeshletMaxPrimitives : 0;
            for (uint32_t& pad : m_mappedObjectData[frameIndex][index].padding) {
                pad = 0;
            }
        }
        ...
        const D3D12_RESOURCE_STATES indirectState = m_indirectBuffersInitialized[frameIndex] ? D3D12_RESOURCE_STATE_INDIRECT_ARGUMENT : D3D12_RESOURCE_STATE_COMMON;

        D3D12_RESOURCE_BARRIER barriers[2] = {};
        barriers[0].Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION; barriers[0].Transition.pResource = m_counterBuffer[frameIndex].Get(); barriers[0].Transition.StateBefore = indirectState; barriers[0].Transition.StateAfter = D3D12_RESOURCE_STATE_COPY_DEST; barriers[0].Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
        barriers[1].Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION; barriers[1].Transition.pResource = m_commandBuffer[frameIndex].Get(); barriers[1].Transition.StateBefore = indirectState; barriers[1].Transition.StateAfter = D3D12_RESOURCE_STATE_UNORDERED_ACCESS; barriers[1].Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
        commandList->ResourceBarrier(2, barriers);

        commandList->CopyBufferRegion(m_counterBuffer[frameIndex].Get(), 0, m_zeroBuffer.Get(), 0, sizeof(uint32_t));

        D3D12_RESOURCE_BARRIER uavBarrier = {}; uavBarrier.Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION; uavBarrier.Transition.pResource = m_counterBuffer[frameIndex].Get(); uavBarrier.Transition.StateBefore = D3D12_RESOURCE_STATE_COPY_DEST; uavBarrier.Transition.StateAfter = D3D12_RESOURCE_STATE_UNORDERED_ACCESS; uavBarrier.Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
        ...
        endBarriers[1].Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION; endBarriers[1].Transition.pResource = m_commandBuffer[frameIndex].Get(); endBarriers[1].Transition.StateBefore = D3D12_RESOURCE_STATE_UNORDERED_ACCESS; endBarriers[1].Transition.StateAfter = D3D12_RESOURCE_STATE_INDIRECT_ARGUMENT; endBarriers[1].Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
        commandList->ResourceBarrier(2, endBarriers);
        m_indirectBuffersInitialized[frameIndex] = true;

        commandList->SetPipelineState(meshPipeline);
        commandList->SetGraphicsRootSignature(m_quantaRootSignature.Get());
        commandList->SetGraphicsRootConstantBufferView(0, m_globalCB[frameIndex]->GetGPUVirtualAddress());
        commandList->SetGraphicsRootConstantBufferView(1, m_materialCB[frameIndex]->GetGPUVirtualAddress());
