...
void SkyboxPass::Initialize(ID3D12Device* device) {
    ...
    // The editor composites through an HDR target, so the skybox needs a
    // matching variant; the asset preview windows still draw straight to the
    // R8G8B8A8 back buffer.
    D3D12_GRAPHICS_PIPELINE_STATE_DESC psoHdrDesc = psoDesc;
    psoHdrDesc.RTVFormats[0] = DXGI_FORMAT_R16G16B16A16_FLOAT;
    if (FAILED(device->CreateGraphicsPipelineState(&psoHdrDesc, IID_PPV_ARGS(&m_psoHdr)))) {
        m_psoHdr.Reset();
    }
}
...
void SkyboxPass::LoadHDR(ID3D12Device* device, ID3D12GraphicsCommandList* cmdList, const wchar_t* filename) {
    ...
    // The caller reports the failure; leaving m_skyTexture null makes
    // GetHDRResource() return null so the gradient fallback takes over.
    // (The old retry here passed the same literal path and never did anything.)
    if (FAILED(DirectX::LoadFromHDRFile(filename, nullptr, image))) {
        return;
    }
...
void SkyboxPass::Render(ID3D12GraphicsCommandList* cmdList, ID3D12Device* device, Mesh* cubeMesh, Camera& camera, ID3D12DescriptorHeap* bindlessHeap, uint32_t hdrIndex, const DirectX::XMFLOAT4& zenithColor, const DirectX::XMFLOAT4& horizonColor, bool hdrTarget) {
    if (!cubeMesh || !m_pso) return;

    cmdList->SetPipelineState((hdrTarget && m_psoHdr) ? m_psoHdr.Get() : m_pso.Get());
