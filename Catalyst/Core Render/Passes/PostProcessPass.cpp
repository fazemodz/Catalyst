...
void PostProcessPass::CreateOffscreenRenderTarget(ID3D12Device* device, int width, int height) {
    ...
    if (!m_hdrSrvHeap) { D3D12_DESCRIPTOR_HEAP_DESC srvDesc = {}; srvDesc.NumDescriptors = 3; srvDesc.Type = D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV; srvDesc.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE; ThrowIfFailed(device->CreateDescriptorHeap(&srvDesc, IID_PPV_ARGS(&m_hdrSrvHeap))); }
...
void PostProcessPass::CreatePipeline(ID3D12Device* device) {
    ...
    range.RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV; range.NumDescriptors = 3; range.BaseShaderRegister = 0; range.RegisterSpace = 0; range.OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;
    ...
    rp[1].ParameterType = D3D12_ROOT_PARAMETER_TYPE_32BIT_CONSTANTS; rp[1].Constants.ShaderRegister = 0; rp[1].Constants.RegisterSpace = 0; rp[1].Constants.Num32BitValues = 16; rp[1].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;
...
void PostProcessPass::SetRaytracingInputs(ID3D12Device* device, ID3D12Resource* shadowMask, ID3D12Resource* reflections) {
    if (device == nullptr || !m_hdrSrvHeap) {
        return;
    }

    const UINT stride = device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
    D3D12_CPU_DESCRIPTOR_HANDLE handle = m_hdrSrvHeap->GetCPUDescriptorHandleForHeapStart();
    ...
    if (shadowMask != nullptr) {
        srv.Format = shadowMask->GetDesc().Format;
        device->CreateShaderResourceView(shadowMask, &srv, handle);
    }

    handle.ptr += stride;
    if (reflections != nullptr) {
        srv.Format = reflections->GetDesc().Format;
        device->CreateShaderResourceView(reflections, &srv, handle);
    }

    m_hasRaytracingInputs = (shadowMask != nullptr && reflections != nullptr);
}

void PostProcessPass::Render(ID3D12GraphicsCommandList* commandList, const Camera& camera, const std::vector<GameObject>& gameObjects, const PostProcessSettings& globalPP, int width, int height, int sceneWidth, int sceneHeight, bool raytracingActive, float shadowStrength, float aoStrength) {
    ...
    const float uvScaleX = (width  > 0) ? std::clamp(static_cast<float>(sceneWidth)  / static_cast<float>(width),  0.0f, 1.0f) : 1.0f;
    const float uvScaleY = (height > 0) ? std::clamp(static_cast<float>(sceneHeight) / static_cast<float>(height), 0.0f, 1.0f) : 1.0f;
    const float raytracingEnabled = (raytracingActive && m_hasRaytracingInputs) ? 1.0f : 0.0f;
    float ppData[16] = { activePP.exposure, activePP.colorTint.x, activePP.colorTint.y, activePP.colorTint.z, activePP.bloomThreshold, activePP.bloomIntensity, static_cast<float>(width), static_cast<float>(height), uvScaleX, uvScaleY, raytracingEnabled, shadowStrength, aoStrength, 0.0f, 0.0f, 0.0f };
    commandList->SetGraphicsRoot32BitConstants(1, 16, ppData, 0); commandList->DrawInstanced(3, 1, 0, 0);
}
