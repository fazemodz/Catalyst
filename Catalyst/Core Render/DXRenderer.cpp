...
bool LoadBlueprintRuntimeData(const std::wstring& assetPath, BlueprintRuntimeData& outRuntimeData) {
    ...
    const Json::Value* nodesValue = Json::FindField(rootValue, "nodes");
    if (nodesValue != nullptr && nodesValue->type == Json::ValueType::Array) {
        for (const Json::Value& nodeValue : nodesValue->arrayValue) {
            ...
            const int nodeId = Json::GetInt(nodeValue, "id", 0);
            const std::string nodeTypeId = Json::GetString(nodeValue, "nodeTypeId");
            runtimeNodes[nodeId] = {
                nodeTypeId,
                ResolveBlueprintReferencePath(assetPath, Json::GetString(nodeValue, "assetPath"))
            };
    ...
    const Json::Value* linksValue = Json::FindField(rootValue, "links");
    if (linksValue != nullptr && linksValue->type == Json::ValueType::Array) {
        ...
        for (const Json::Value& linkValue : linksValue->arrayValue) {
            ...
            const int fromNodeId = Json::GetInt(linkValue, "fromNodeId", 0);
            const int toNodeId = Json::GetInt(linkValue, "toNodeId", 0);
            ...
            const std::string fromPinKind = Json::GetString(linkValue, "fromPinKind", "Exec");
            const std::string toPinKind = Json::GetString(linkValue, "toPinKind", "Exec");
    ...
    const Json::Value* componentsValue = Json::FindField(rootValue, "components");
    if (componentsValue != nullptr && componentsValue->type == Json::ValueType::Array) {
        ...
        for (const Json::Value& componentValue : componentsValue->arrayValue) {
            ...
            component.kind = Json::GetString(componentValue, "kind", "StaticMesh");
            component.assetPath = ResolveBlueprintReferencePath(assetPath, Json::GetString(componentValue, "assetPath"));
            ...
            component.triggerShape = PhysicsColliderShapeFromString(Json::GetString(componentValue, "triggerShape", "Box"));
...
std::string DXRenderer::BuildSceneDocument(const std::vector<GameObject>& objectsToSave, const std::wstring& projectRoot) const {
    ...
    for (size_t objectIndex = 0; objectIndex < objectsToSave.size(); ++objectIndex) {
        ...
        outFile << "    {\n";
        outFile << "      \"name\": \"" << Json::Escape(object.name) << "\",\n";
        outFile << "      \"tag\": \"" << Json::Escape(object.tag) << "\",\n";
        outFile << "      \"animationState\": \"" << Json::Escape(object.animationState) << "\",\n";
        ...
        outFile << "      \"assetName\": \"" << Json::Escape(object.asset ? object.asset->name : "") << "\",\n";
        outFile << "      \"assetSource\": \"" << Json::Escape(WideToUtf8(assetSourcePath)) << "\",\n";
        outFile << "      \"blueprintAsset\": \"" << Json::Escape(WideToUtf8(blueprintAssetPath)) << "\",\n";
        outFile << "      \"components\": [\n";
        for (size_t componentIndex = 0; componentIndex < object.nativeScriptComponents.size(); ++componentIndex) {
            ...
            outFile << "          \"className\": \"" << Json::Escape(component.className) << "\",\n";
            outFile << "          \"enabled\": " << (component.enabled ? "true" : "false") << "\n";
            outFile << "        }" << (componentIndex + 1 < object.nativeScriptComponents.size() ? "," : "") << "\n";
        }
        outFile << "      ],\n";
        outFile << "      \"position\": ";
        Json::WriteFloat3(outFile, object.position);
        outFile << ",\n";
        outFile << "      \"rotation\": ";
        Json::WriteFloat3(outFile, object.rotation);
        outFile << ",\n";
        outFile << "      \"scale\": ";
        Json::WriteFloat3(outFile, object.scale);
        outFile << ",\n";
        outFile << "      \"roughness\": " << object.roughness << ",\n";
        outFile << "      \"localPosition\": ";
        Json::WriteFloat3(outFile, object.localPosition);
        outFile << ",\n";
        outFile << "      \"localRotation\": ";
        Json::WriteFloat3(outFile, object.localRotation);
        outFile << ",\n";
        outFile << "      \"localScale\": ";
        Json::WriteFloat3(outFile, object.localScale);
        outFile << ",\n";
        outFile << "      \"color\": ";
        Json::WriteFloat4(outFile, object.color);
        outFile << ",\n";
        outFile << "      \"skyHorizonColor\": ";
        Json::WriteFloat4(outFile, object.skyHorizonColor);
        outFile << ",\n";
        outFile << "      \"assignedMaterial\": \"" << Json::Escape(WideToUtf8(assignedMaterialPath)) << "\",\n";
        outFile << "      \"overrideTextures\": {\n";
        outFile << "        \"albedo\": \"" << Json::Escape(WideToUtf8(MakeProjectRelativePath(projectRoot, GetCachedTexturePath(object.overrideAlbedo)))) << "\",\n";
        outFile << "        \"normal\": \"" << Json::Escape(WideToUtf8(MakeProjectRelativePath(projectRoot, GetCachedTexturePath(object.overrideNormal)))) << "\",\n";
        outFile << "        \"metallic\": \"" << Json::Escape(WideToUtf8(MakeProjectRelativePath(projectRoot, GetCachedTexturePath(object.overrideMetallic)))) << "\",\n";
        outFile << "        \"roughness\": \"" << Json::Escape(WideToUtf8(MakeProjectRelativePath(projectRoot, GetCachedTexturePath(object.overrideRoughness)))) << "\",\n";
        outFile << "        \"ao\": \"" << Json::Escape(WideToUtf8(MakeProjectRelativePath(projectRoot, GetCachedTexturePath(object.overrideAO)))) << "\"\n";
        outFile << "      },\n";
        outFile << "      \"lightIntensity\": " << object.lightIntensity << ",\n";
        outFile << "      \"postProcess\": {\n";
        outFile << "        \"exposure\": " << object.ppSettings.exposure << ",\n";
        outFile << "        \"colorTint\": ";
        Json::WriteFloat3(outFile, object.ppSettings.colorTint);
        outFile << ",\n";
        ...
        outFile << "          \"velocity\": ";
        Json::WriteFloat3(outFile, object.physics.rigidBody.velocity);
        outFile << "\n";
        outFile << "        },\n";
        outFile << "        \"collider\": {\n";
        ...
        Json::WriteFloat3(outFile, object.physics.collider.centerOffset);
        outFile << ",\n";
        outFile << "          \"boxExtents\": ";
        Json::WriteFloat3(outFile, object.physics.collider.boxExtents);
        outFile << ",\n";
...
std::string DXRenderer::BuildMaterialDocument(const Material& material) const {
    ...
    file << "  \"version\": 2,\n";
    file << "  \"baseColor\": ";
    Json::WriteFloat4(file, material.baseColor);
    file << ",\n";
    file << "  \"textures\": {\n";
    file << "    \"albedo\": \"" << Json::Escape(NormalizeLinkedMaterialPath(material.albedoPath)) << "\",\n";
    file << "    \"normal\": \"" << Json::Escape(NormalizeLinkedMaterialPath(material.normalPath)) << "\",\n";
    file << "    \"roughness\": \"" << Json::Escape(NormalizeLinkedMaterialPath(material.roughnessPath)) << "\"\n";
    file << "  }\n";
    file << "}\n";
    return file.str();
}
...
bool DXRenderer::SaveScriptValue(const std::string& slotName, const std::string& key, const std::string& value) {
    ...
    if (fs::exists(savePath)) {
        ...
        Json::Parser parser(content);
        if (parser.Parse(rootValue) && rootValue.type == Json::ValueType::Object) {
            const Json::Value* entriesValue = Json::FindField(rootValue, "entries");
            if (entriesValue != nullptr && entriesValue->type == Json::ValueType::Array) {
                for (const Json::Value& entry : entriesValue->arrayValue) {
                    if (entry.type != Json::ValueType::Object) {
                        continue;
                    }
                    entries[Json::GetString(entry, "key")] = Json::GetString(entry, "value");
                }
            }
        }
    }
    ...
    for (const auto& [entryKey, entryValue] : entries) {
        outputFile << "    {\"key\": \"" << Json::Escape(entryKey) << "\", \"value\": \"" << Json::Escape(entryValue) << "\"}";
...
bool DXRenderer::LoadScriptValue(const std::string& slotName, const std::string& key, std::string& outValue) const {
    ...
    for (const Json::Value& entry : entriesValue->arrayValue) {
        ...
        if (Json::GetString(entry, "key") == key) {
            outValue = Json::GetString(entry, "value");
            return true;
        }
    }
...
void DXRenderer::ScriptHostLog(void* userData, const char* message) {
    ...
    DebugLog(msg);
    OutputLog::Add(msg, OutputLog::Level::Info);
}

void DXRenderer::ScriptHostLogWarning(void* userData, const char* message) {
    const DXRenderer* renderer = static_cast<const DXRenderer*>(userData);
    (void)renderer;
    const char* msg = message != nullptr ? message : "";
    DebugLog(std::string("[Warning] ") + msg);
    OutputLog::Add(msg, OutputLog::Level::Warning);
}

void DXRenderer::ScriptHostLogError(void* userData, const char* message) {
    const DXRenderer* renderer = static_cast<const DXRenderer*>(userData);
    (void)renderer;
    const char* msg = message != nullptr ? message : "";
    DebugLog(std::string("[Error] ") + msg);
    OutputLog::Add(msg, OutputLog::Level::Error);
}
...
bool DXRenderer::LoadRuntimeWidgetInstance(const std::wstring& assetPath, RuntimeWidgetInstance& outInstance) {
    ...
    // XML authoring fallback: try a sibling .xml file
    if (!loadedCookedAsset) {
        fs::path xmlPath = fs::path(assetPath);
        xmlPath.replace_extension(L".xml");
        std::error_code xmlEc;
        if (fs::exists(xmlPath, xmlEc) && !xmlEc) {
            loadedCookedAsset = CookedUIBlueprint::CompileFromXmlFile(xmlPath.wstring(), cookedAsset, nullptr);
        }
    }
    ...
    for (uint32_t instructionIndex = firstInstructionIndex; instructionIndex < lastInstructionIndex; ++instructionIndex) {
        ...
        case CookedUIBlueprint::Opcode::Print:
            if (!instruction.text.empty()) {
                OutputLog::Add(instruction.text, OutputLog::Level::Info);
            }
            break;
...
void DXRenderer::ClearProjectRuntimeAssets() {
    ...
    for (auto& cached : m_textureCache) {
        ReleaseCachedTexture(cached.second);
    }
...
bool DXRenderer::LoadStartupSceneForProject(const std::wstring& projectFilePath) {
    ...
    LoadProjectSettingsFor(normalizedProjectFilePath);
...
bool DXRenderer::LoadSceneFromMap(const std::wstring& scenePath, const std::wstring& projectRoot) {
    ...
    const Json::Value* objectsValue = Json::FindField(rootValue, "objects");
    if (!objectsValue || objectsValue->type != Json::ValueType::Array) {
        ResetSceneToDefaults();
        return false;
    }

    for (const Json::Value& serializedObject : objectsValue->arrayValue) {
        if (serializedObject.type != Json::ValueType::Object) {
            continue;
        }

        GameObject sceneObject;
        sceneObject.name = Json::GetString(serializedObject, "name", "GameObject");
        sceneObject.tag = Json::GetString(serializedObject, "tag");
        sceneObject.animationState = Json::GetString(serializedObject, "animationState");
        sceneObject.layer = static_cast<uint32_t>(Json::GetInt(serializedObject, "layer", 0));
        sceneObject.enabled = Json::GetBool(serializedObject, "enabled", true);
        sceneObject.id = static_cast<uint64_t>(Json::GetInt(serializedObject, "id", static_cast<int>(sceneObject.id)));
        sceneObject.parentId = static_cast<uint64_t>(Json::GetInt(serializedObject, "parentId", 0));
        sceneObject.inheritParentTransform = Json::GetBool(serializedObject, "inheritParentTransform", sceneObject.parentId != 0);
        sceneObject.type = ObjectTypeFromString(Json::GetString(serializedObject, "type", "Mesh"));
        sceneObject.position = Json::GetFloat3(serializedObject, "position", {0.0f, 0.0f, 0.0f});
        sceneObject.rotation = Json::GetFloat3(serializedObject, "rotation", {0.0f, 0.0f, 0.0f});
        sceneObject.scale = Json::GetFloat3(serializedObject, "scale", {1.0f, 1.0f, 1.0f});
        sceneObject.roughness = Json::GetNumber(serializedObject, "roughness", 1.0f);
        sceneObject.localPosition = Json::GetFloat3(serializedObject, "localPosition", sceneObject.position);
        sceneObject.localRotation = Json::GetFloat3(serializedObject, "localRotation", sceneObject.rotation);
        sceneObject.localScale = Json::GetFloat3(serializedObject, "localScale", sceneObject.scale);
        sceneObject.color = Json::GetFloat4(serializedObject, "color", {1.0f, 1.0f, 1.0f, 1.0f});
        sceneObject.skyHorizonColor = Json::GetFloat4(serializedObject, "skyHorizonColor", {1.0f, 1.0f, 1.0f, 1.0f});
        sceneObject.lightIntensity = Json::GetNumber(serializedObject, "lightIntensity", 1.0f);

        const Json::Value* postProcessValue = Json::FindField(serializedObject, "postProcess");
        if (postProcessValue && postProcessValue->type == Json::ValueType::Object) {
            sceneObject.ppSettings.exposure = Json::GetNumber(*postProcessValue, "exposure", 1.0f);
            sceneObject.ppSettings.colorTint = Json::GetFloat3(*postProcessValue, "colorTint", {1.0f, 1.0f, 1.0f});
            sceneObject.ppSettings.bloomThreshold = Json::GetNumber(*postProcessValue, "bloomThreshold", 1.0f);
            sceneObject.ppSettings.bloomIntensity = Json::GetNumber(*postProcessValue, "bloomIntensity", 0.5f);
            sceneObject.ppSettings.blendRadius = Json::GetNumber(*postProcessValue, "blendRadius", 1.0f);
        }

        const int assetId = Json::GetInt(serializedObject, "assetId", -1);
        const std::wstring assetSourcePath = ResolveSceneReferencePath(projectRoot, Json::GetString(serializedObject, "assetSource"));
        sceneObject.asset = ResolveSceneAsset(assetId, Json::GetString(serializedObject, "assetName"), assetSourcePath);
        sceneObject.blueprintAssetPath = ResolveSceneReferencePath(projectRoot, Json::GetString(serializedObject, "blueprintAsset"));
        const Json::Value* componentsValue = Json::FindField(serializedObject, "components");
        if (componentsValue != nullptr && componentsValue->type == Json::ValueType::Array) {
            for (const Json::Value& componentValue : componentsValue->arrayValue) {
                if (componentValue.type != Json::ValueType::Object) {
                    continue;
                ...
                if (Json::GetString(componentValue, "type") != "NativeScript") {
                    continue;
                }

                NativeScriptComponentDesc component;
                component.id = static_cast<uint64_t>(Json::GetInt(componentValue, "id", static_cast<int>(component.id)));
                component.className = TrimAsciiWhitespace(Json::GetString(componentValue, "className"));
                component.enabled = Json::GetBool(componentValue, "enabled", true);
                sceneObject.nativeScriptComponents.push_back(component);
            }
        }

        const std::string legacyNativeBehavior = TrimAsciiWhitespace(Json::GetString(serializedObject, "nativeBehavior"));
        ...
        const std::wstring materialPath = ResolveSceneReferencePath(projectRoot, Json::GetString(serializedObject, "assignedMaterial"));
        if (!materialPath.empty()) {
            sceneObject.assignedMaterial = LoadMaterialAsset(materialPath);
        }

        const Json::Value* overrideTextures = Json::FindField(serializedObject, "overrideTextures");
        if (overrideTextures && overrideTextures->type == Json::ValueType::Object) {
            const std::wstring albedoPath = ResolveSceneReferencePath(projectRoot, Json::GetString(*overrideTextures, "albedo"));
            const std::wstring normalPath = ResolveSceneReferencePath(projectRoot, Json::GetString(*overrideTextures, "normal"));
            const std::wstring metallicPath = ResolveSceneReferencePath(projectRoot, Json::GetString(*overrideTextures, "metallic"));
            const std::wstring roughnessPath = ResolveSceneReferencePath(projectRoot, Json::GetString(*overrideTextures, "roughness"));
            const std::wstring aoPath = ResolveSceneReferencePath(projectRoot, Json::GetString(*overrideTextures, "ao"));
        ...
        const Json::Value* physicsValue = Json::FindField(serializedObject, "physics");
        if (physicsValue && physicsValue->type == Json::ValueType::Object) {
            const Json::Value* rigidBodyValue = Json::FindField(*physicsValue, "rigidBody");
            if (rigidBodyValue && rigidBodyValue->type == Json::ValueType::Object) {
                sceneObject.physics.rigidBody.enabled = Json::GetBool(*rigidBodyValue, "enabled", false);
                sceneObject.physics.rigidBody.bodyType =
                    PhysicsBodyTypeFromString(Json::GetString(*rigidBodyValue, "bodyType", "Dynamic"));
                sceneObject.physics.rigidBody.useGravity = Json::GetBool(*rigidBodyValue, "useGravity", true);
                sceneObject.physics.rigidBody.mass = Json::GetNumber(*rigidBodyValue, "mass", 1.0f);
                sceneObject.physics.rigidBody.linearDamping = Json::GetNumber(*rigidBodyValue, "linearDamping", 0.12f);
                sceneObject.physics.rigidBody.restitution = Json::GetNumber(*rigidBodyValue, "restitution", 0.2f);
                sceneObject.physics.rigidBody.velocity = Json::GetFloat3(*rigidBodyValue, "velocity", {0.0f, 0.0f, 0.0f});
            }

            const Json::Value* colliderValue = Json::FindField(*physicsValue, "collider");
            if (colliderValue && colliderValue->type == Json::ValueType::Object) {
                sceneObject.physics.collider.enabled = Json::GetBool(*colliderValue, "enabled", false);
                sceneObject.physics.collider.shape =
                    PhysicsColliderShapeFromString(Json::GetString(*colliderValue, "shape", "Box"));
                sceneObject.physics.collider.isTrigger = Json::GetBool(*colliderValue, "isTrigger", false);
                sceneObject.physics.collider.centerOffset =
                    Json::GetFloat3(*colliderValue, "centerOffset", {0.0f, 0.0f, 0.0f});
                sceneObject.physics.collider.boxExtents =
                    Json::GetFloat3(*colliderValue, "boxExtents", {0.5f, 0.5f, 0.5f});
                sceneObject.physics.collider.sphereRadius =
                    Json::GetNumber(*colliderValue, "sphereRadius", 0.5f);
            }
    ...
    // If the loaded scene has no sky object (legacy scene or stripped save),
    // inject the default Sky Atmosphere so it is present for this session.
    bool hasSky = false;
    for (const auto& o : m_gameObjects)
        if (o.name.find("Sky") != std::string::npos) { hasSky = true; break; }

    if (!hasSky) {
        if (Asset* cubeAsset = FindAssetById(0)) {
            GameObject skybox;
            skybox.name            = "Sky Atmosphere";
            skybox.position        = {0.0f, -9999.0f, 0.0f};
            skybox.scale           = {1.0f, 1.0f, 1.0f};
...
Texture* DXRenderer::LoadTextureAsset(const std::wstring& path) {
    ...
    if (cached != m_textureCache.end()) {
        // A cached null is a remembered failure; without it a texture that
        // cannot be loaded is re-read from disk and re-uploaded every frame.
    ...
    try {
        ...
        OutputLog::Add("Failed to load texture: " + WideToUtf8(normalizedPath), OutputLog::Level::Error);
        m_textureCache[normalizedPath] = nullptr;
        return nullptr;
    }

    texture->SetBindlessIndex(m_bindlessManager.AddTexture(m_device.Get(), texture->GetResource()));
    if (texture->GetBindlessIndex() < 0) {
        OutputLog::Add("Bindless texture heap is full (" + std::to_string(m_bindlessManager.GetCapacity()) +
                       " slots); texture not loaded: " + WideToUtf8(normalizedPath), OutputLog::Level::Error);
        m_textureCache[normalizedPath] = nullptr;
        return nullptr;
    }
...
void DXRenderer::ReleaseCachedTexture(const std::shared_ptr<Texture>& texture) {
    if (texture) {
        m_bindlessManager.ReleaseTexture(texture->GetBindlessIndex());
    }
}

void DXRenderer::ApplyEditorPreferences() {
    m_editorPreferences.ClampToValidRanges();
    m_camera.SetMoveSpeed(m_editorPreferences.cameraMoveSpeed);
    m_camera.SetTurnSpeed(m_editorPreferences.cameraTurnSpeed);
    m_editorUI.State.buildConfig = m_editorPreferences.buildRelease
        ? EditorUIState::BuildConfig::Release
        : EditorUIState::BuildConfig::Debug;
}

void DXRenderer::ApplyProjectSettings() {
    m_projectSettings.ClampToValidRanges();
    m_globalPP.exposure       = m_projectSettings.exposure;
    m_globalPP.colorTint      = m_projectSettings.colorTint;
    m_globalPP.bloomThreshold = m_projectSettings.bloomThreshold;
    m_globalPP.bloomIntensity = m_projectSettings.bloomIntensity;
    m_physicsSystem.SetGravity(m_projectSettings.gravity);
    m_physicsSystem.SetFixedTimeStep(m_projectSettings.fixedTimeStep);
}

bool DXRenderer::SaveEditorPreferences() {
    ApplyEditorPreferences();
    const bool saved = EditorSettingsIO::SavePreferences(m_editorPreferences);
    if (saved) {
        m_hasSavedEditorPreferences = true;
    }
    return saved;
}

bool DXRenderer::SaveProjectSettings() {
    const std::wstring projectFilePath = ResolveActiveProjectFilePath();
    if (projectFilePath.empty()) {
        return false;
    }
    ...
    return UpdateProjectSettings(projectFilePath, m_projectSettings);
}

void DXRenderer::LoadProjectSettingsFor(const std::wstring& projectFilePath) {
    if (projectFilePath.empty()) {
        m_projectSettings = ProjectSettings{};
    } else {
        m_projectSettings = ParseProjectFile(projectFilePath).Settings;
    }
    ApplyProjectSettings();
}
...
void DXRenderer::Initialize(HWND hwnd, int width, int height) {
    ...
    m_nativeScriptHostApi.logWarning = &DXRenderer::ScriptHostLogWarning;
    m_nativeScriptHostApi.logError = &DXRenderer::ScriptHostLogError;
    ...
    if (m_raytracePass.Initialize(m_device.Get())) {
        UINT64 timestampFrequency = 0;
        if (SUCCEEDED(m_commandQueue->GetTimestampFrequency(&timestampFrequency))) {
            m_raytracePass.SetTimestampFrequency(timestampFrequency);
        }
        m_raytracePass.OnResize(m_device.Get(), m_width, m_height);
        m_postProcessPass.SetRaytracingInputs(m_device.Get(), m_raytracePass.GetShadowMask(),
                                              m_raytracePass.GetReflections());
        OutputLog::Add("Raytracing enabled (DXR 1.1 inline RayQuery).");
    } else {
        OutputLog::Add("Raytracing unavailable: " + WideToUtf8(m_raytracePass.GetUnavailableReason()),
                       OutputLog::Level::Warning);
    }
    ...
    const std::wstring skyHdrPath = CatalystRender::ResolveShaderPath(L"Assets/sky.hdr");
    m_skyboxPass.LoadHDR(m_device.Get(), m_commandList.Get(), skyHdrPath.c_str());
    ...
    if (m_skyboxPass.GetHDRResource()) {
        ...
        if (skyboxIndex >= 0) {
            ...
            OutputLog::Add("Skybox HDR loaded but no bindless slot was free; falling back to a flat gradient sky.",
                           OutputLog::Level::Error);
        }
    } else {
        OutputLog::Add("Could not load Assets/sky.hdr (looked for \"" + WideToUtf8(skyHdrPath) +
                       "\"). The skybox will fall back to a flat gradient.", OutputLog::Level::Error);
    }
    ...
    m_hasSavedEditorPreferences = EditorSettingsIO::LoadPreferences(m_editorPreferences);
    ApplyEditorPreferences();
    m_scriptCameraFov = m_editorPreferences.viewportFov;
    ...
    OutputLog::Add("Catalyst Engine started.");
    OutputLog::Add("DirectX 12 renderer initialized.");
}

void DXRenderer::LoadEditorXmlFiles() {
    wchar_t exePath[MAX_PATH] = {};
    GetModuleFileNameW(nullptr, exePath, MAX_PATH);
    fs::path exeDir = fs::path(exePath).parent_path();

    fs::path stylePath  = exeDir / L"editor_theme.xmlstyle";
    fs::path layoutPath = exeDir / L"editor_layout.xml";

    // Track file times to avoid unnecessary re-parses
    static fs::file_time_type lastStyleTime  = {};
    static fs::file_time_type lastLayoutTime = {};

    std::error_code ec;
    bool styleChanged  = false;
    bool layoutChanged = false;

    if (fs::exists(stylePath, ec)) {
        auto t = fs::last_write_time(stylePath, ec);
        if (!ec && t != lastStyleTime) { lastStyleTime = t; styleChanged = true; }
    }
    if (fs::exists(layoutPath, ec)) {
        auto t = fs::last_write_time(layoutPath, ec);
        if (!ec && t != lastLayoutTime) { lastLayoutTime = t; layoutChanged = true; }
    }

    if (styleChanged)
        m_editorStyleSheet = UIXML::StyleSheet::ParseFromFile(stylePath.wstring());

    if (layoutChanged || styleChanged)
        m_editorLayoutDoc = fs::exists(layoutPath, ec)
            ? UIXML::ParseFromFile(layoutPath.wstring(), &m_editorStyleSheet)
            : UIXML::Document{};
}

void DXRenderer::ReloadUIXML() {
    m_editorStyleSheet = {};
    m_editorLayoutDoc  = {};
    LoadEditorXmlFiles();
    // Also evict all runtime widget instances so they reload their XML on next use
...
void DXRenderer::OnResize(int width, int height) {
    ...
    for (UINT64& frameFenceValue : m_frameFenceValues) {
        frameFenceValue = 0; // FlushGPU above retired every in-flight frame
    }
    ...
    if (m_raytracePass.IsAvailable()) {
        m_raytracePass.OnResize(m_device.Get(), width, height);
        m_postProcessPass.SetRaytracingInputs(m_device.Get(), m_raytracePass.GetShadowMask(),
                                              m_raytracePass.GetReflections());
    }
}
...
void DXRenderer::Render() {
    // Hot-reload editor XML files once per second (Ctrl+Shift+R forces immediate reload)
    {
        ULONGLONG now = GetTickCount64();
        if (now - m_lastXmlReloadTick >= 1000) {
            m_lastXmlReloadTick = now;
            LoadEditorXmlFiles();
        }
    }

    // Reuse this frame's allocator only after the GPU has finished the last
    ...
    // Panel dimensions — driven by editor_layout.xml when present
    float topH    = 65.0f;
    float rightW  = 350.0f;
    float bottomH = 250.0f;
    if (m_editorLayoutDoc.valid) {
        const UIXML::Node& root = m_editorLayoutDoc.root;
        auto findPanel = [&](const char* id, float& outSize, bool useWidth) {
            for (const auto& child : root.children) {
                if (child.id == id) {
                    outSize = useWidth ? child.w : child.h;
                    return;
                }
            }
        };
        findPanel("menubar",  topH,    false);
        findPanel("outliner", rightW,  true);
        findPanel("bottombar", bottomH, false);
        // toolbar height adds to topH
        for (const auto& child : root.children) {
            if (child.id == "toolbar") { topH += child.h; break; }
        }
    }
    if (m_hasSavedEditorPreferences) {
        rightW  = m_editorPreferences.rightPanelWidth;
        bottomH = m_editorPreferences.bottomPanelHeight;
    }
    ...
    if (deltaTime > 0.0f) {
        // Exponential moving average; ~0.1 keeps it responsive without jitter.
        const float instantMs = deltaTime * 1000.0f;
        m_smoothedFrameMs = (m_smoothedFrameMs <= 0.0f) ? instantMs
                                                        : (m_smoothedFrameMs * 0.9f + instantMs * 0.1f);
        m_smoothedFps = (m_smoothedFrameMs > 0.0001f) ? (1000.0f / m_smoothedFrameMs) : 0.0f;
    }
...

    {
        ...
        if (ConsumeAppLaunchRequest(req)) {
            if (req.play) {
            ...
            } else if (!req.projectFilePath.empty()) {
                const std::wstring projPath = NormalizeAssetPath(req.projectFilePath);
                AddRecentProject(projPath);
                m_editorUI.State.currentProjectFile   = projPath;
                m_editorUI.State.currentProjectFolder = fs::path(projPath).parent_path().wstring();
                m_editorUI.State.currentMapPath.clear();
                m_editorUI.State.currentBrowserPath   = m_editorUI.State.currentProjectFolder + L"\\Assets";
                m_editorUI.State.triggerEditorSwap    = true;
                m_editorUI.State.editorSwapProjName   = fs::path(projPath).stem().wstring();
                m_editorUI.State.recentsLoaded        = false;
            }
        }
    }
    ...
    if (m_engineState == EngineState::Launcher) {
        ...
        if (!m_editorUI.State.showActorAssetViewer &&
            ...
            sceneMouseInViewport =
                !m_editorUI.State.showSettingsWindow &&
            ...
            if (!m_editorUI.State.isPlaying && !m_editorUI.State.showSettingsWindow) {
    ...
    if (m_engineState == EngineState::Editor && !showBlueprintEditor) {
        if (showMaterialAssetViewer) {
            ...
            const bool viewportInteractive = !m_editorUI.State.showSettingsWindow;
            if (viewportInteractive &&
                m_editorUI.State.selectedObj >= 0 && m_editorUI.State.selectedObj < static_cast<int>(m_gameObjects.size())) {
            ...
            const bool raytracingActive = m_raytracePass.IsAvailable();
            m_raytracePass.BeginGBuffer(m_commandList.Get());

            D3D12_CPU_DESCRIPTOR_HANDLE hdrRtv = m_postProcessPass.GetRTV();
            D3D12_CPU_DESCRIPTOR_HANDLE sceneTargets[3] = { hdrRtv, {}, {} };
            if (raytracingActive) {
                sceneTargets[1] = m_raytracePass.GetGBufferRTV();
                sceneTargets[2] = m_raytracePass.GetPositionRTV();
            }
            m_commandList->OMSetRenderTargets(raytracingActive ? 3 : 1, sceneTargets, FALSE, &dsv);
            ...
            // Always render the skybox; fall back to neutral white when no sky
            // object is present (e.g. legacy scenes or brand-new projects).
            const XMFLOAT4 skyZenith  = skyObject ? skyObject->color           : XMFLOAT4{1.f, 1.f, 1.f, 1.f};
            const XMFLOAT4 skyHorizon = skyObject ? skyObject->skyHorizonColor : XMFLOAT4{1.f, 1.f, 1.f, 1.f};
            m_skyboxPass.Render(
                m_commandList.Get(),
                m_device.Get(),
                m_primitives["Cube"],
                ...
                skyZenith,
                skyHorizon,
                true); // main scene composites through the HDR target

            m_raytracePass.EndGBuffer(m_commandList.Get());

            if (raytracingActive) {
                // The depth target doubles as the raytracing G-buffer depth, so
                // it is handed to the compute pass and handed straight back.
                D3D12_RESOURCE_BARRIER depthToRead = {};
                depthToRead.Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
                depthToRead.Transition.pResource   = m_depthBuffer.Get();
                ...
                m_raytracePass.BuildAccelerationStructures(m_commandList.Get(), m_device.Get(), m_gameObjects, m_frameIndex);

                RaytracePass::TraceParams traceParams;
                traceParams.lightDirection     = lightDir;
                traceParams.skyZenith          = {skyZenith.x, skyZenith.y, skyZenith.z};
                traceParams.skyHorizon         = {skyHorizon.x, skyHorizon.y, skyHorizon.z};
                traceParams.surfaceBias        = m_projectSettings.raytraceSurfaceBias;
                traceParams.reflectionDistance = m_projectSettings.raytraceReflectionDistance;
                traceParams.reflectionIntensity= m_projectSettings.raytraceReflectionIntensity;
                traceParams.aoRadius           = m_projectSettings.raytraceAoRadius;
                traceParams.aoSamples          = m_projectSettings.raytraceAoSamples;
                traceParams.aoStrength         = m_projectSettings.raytraceAoStrength;
                traceParams.shadowSoftness     = m_projectSettings.raytraceShadowSoftness;
                traceParams.shadowSamples      = m_projectSettings.raytraceShadowSamples;
                traceParams.shadowsEnabled     = m_projectSettings.raytraceShadowsEnabled;
                traceParams.reflectionsEnabled = m_projectSettings.raytraceReflectionsEnabled;
                traceParams.aoEnabled          = m_projectSettings.raytraceAoEnabled;
                m_raytracePass.Render(m_commandList.Get(), m_device.Get(), m_camera, m_depthBuffer.Get(),
                                      static_cast<int>(viewW), static_cast<int>(viewH), traceParams, m_frameIndex);

                D3D12_RESOURCE_BARRIER depthToWrite = depthToRead;
            ...
            m_commandList->OMSetRenderTargets(1, &rtv, FALSE, &dsv);
            // QuantaMeshPass rebinds the viewport to (0,0,viewW,viewH) for its own
            // rasterisation, so restore the on-screen viewport rect before the
            // composite or the scene lands in the top-left corner of the window.
            m_commandList->RSSetViewports(1, &sceneViewport);
            m_commandList->RSSetScissorRects(1, &sceneScissor);
            m_postProcessPass.Render(m_commandList.Get(), m_camera, m_gameObjects, m_globalPP, m_width, m_height,
                                     ...
                                     m_raytracePass.HasTracedThisFrame() && m_projectSettings.raytraceEnabled,
                                     m_projectSettings.raytraceShadowStrength,
                                     m_projectSettings.raytraceAoStrength);
            ID3D12DescriptorHeap* bindlessHeaps[] = { m_bindlessManager.GetHeap() };
            m_commandList->SetDescriptorHeaps(1, bindlessHeaps);
        }
    }
    ...
    m_uiRenderer.Render(m_commandList.Get(), m_uiDrawList, w, h, m_bindlessManager.GetHeap(), m_frameIndex);
    m_uiDrawList.Clear();
    ...
    // Tag this frame with a fence value and move on; the next time this frame
    // slot comes around, WaitForFrame() blocks only if the GPU is still behind.
    m_fenceValue++;
...
void DXRenderer::WaitForFrame(UINT frameIndex) {
    const UINT64 target = m_frameFenceValues[frameIndex];
    if (target == 0 || m_fence->GetCompletedValue() >= target) {
        return;
    }
    m_fence->SetEventOnCompletion(target, m_fenceEvent);
