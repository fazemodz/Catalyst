...
BuildRunResult BuildStageAndRunPlayableExe(const std::wstring& repoRoot,
    ...
    // The engine loads the default sky as "Assets/sky.hdr", so it has to land
    // inside the staged Assets folder - and after the project's own assets are
    // copied, so a project shipping its own sky.hdr wins.
    if (!fs::exists(stageRoot / L"Assets" / L"sky.hdr") &&
        !CopyFileIfExists(repo / L"Assets" / L"sky.hdr", stageRoot / L"Assets" / L"sky.hdr", &copyErr)) {
        result.error = copyErr;
        return result;
    }
