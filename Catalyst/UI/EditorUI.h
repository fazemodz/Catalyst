...
struct EditorUIState {
    ...
    int  settingsCategory = 1;          // index into the settings sidebar
    std::string settingsStatus;
    uint32_t settingsStatusColor = 0xFFB0B0B0;
    ...
    int selectedLauncherCategory = 0;
    std::string projectLocationPath = "";
    bool isLocationInputActive = false;
    std::vector<ProjectInfo> recentProjects;
    bool recentsLoaded = false;
    bool showNewProjectDialog = false;
    int  selectedLibraryNav   = 0;
    int  selectedQualityPreset = 1;
    bool featureStarterContent = true;
    bool featureRayTracing     = false;
...
public:
    ...
    void DrawSettingsWindow(DXRenderer* renderer, float w, float h);
    void DrawViewportOverlay(DXRenderer* renderer, float viewportTop, float viewportWidth);
    void OpenSettingsWindow(DXRenderer* renderer, int category);
...
private:
    ...
    // Values captured when the settings window opened, so Cancel can restore.
    EditorPreferences m_settingsPrefsOnOpen;
    ProjectSettings m_settingsProjectOnOpen;
    void DrawNewProjectDialog(DXRenderer* renderer, float w, float h);
