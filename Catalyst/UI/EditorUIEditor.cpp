...
void EditorUI::DrawEditor(DXRenderer* renderer, float w, float h, float topH, float rightW, float bottomH) {
    ...
    auto getColor = [renderer](const char* attr, uint32_t fallback) -> uint32_t {
        const auto& cls = renderer->m_editorStyleSheet.Find("editor");
        auto it = cls.find(attr);
        return (it != cls.end()) ? UIXML::ParseHexColor(it->second.c_str(), fallback) : fallback;
    };
    const uint32_t C_MENUBAR  = getColor("menubar",   0xFF141414);
    const uint32_t C_TOOLBAR  = getColor("toolbar",   0xFF232323);
    const uint32_t C_OUTLINER = getColor("outliner",  0xFF262626);
    const uint32_t C_DETAILS  = getColor("details",   0xFF1E1E1E);
    const uint32_t C_BOTTOM   = getColor("bottom",    0xFF1A1A1A);
    const uint32_t C_DIVIDER  = getColor("divider",   0xFF3A3A3A);
    const uint32_t C_ACCENT   = getColor("accent",    0xFFD77800);
    const uint32_t C_TXTPANEL = getColor("textPanel", 0xFFB0B0B0);
    ...
    float outlinerH = (h - topH) * renderer->GetEditorPreferences().outlinerSplit;
    ...
    {
        std::vector<OutputLog::Entry> newEntries;
        if (OutputLog::Drain(newEntries)) {
            for (auto& entry : newEntries) {
                LogEntry logEntry;
                logEntry.timestamp = std::move(entry.timestamp);
                logEntry.message = std::move(entry.message);
                logEntry.color = entry.level == OutputLog::Level::Error   ? 0xFF4444FF
                               : entry.level == OutputLog::Level::Warning ? 0xFF00DDFF
                               :                                            0xFFCCCCCC;
                State.editorLog.push_back(std::move(logEntry));
            }
            if (State.editorLog.size() > 500) {
                State.editorLog.erase(State.editorLog.begin(),
                                      State.editorLog.begin() + static_cast<ptrdiff_t>(State.editorLog.size() - 500));
            }
            State.logScrollToBottom = true;
        }
    }
    ...
    drawList.AddRectFilled(0, 0, w, 25.0f, C_MENUBAR);
    ...
    auto MeasureLabelWidth = [&](const std::string& text) {
        ...
        for (size_t i = 0; i < text.size();) {
            width += fontMgr.GetGlyph(TextUtf8::NextCodepoint(text, i)).Advance;
        }
        return width;
    };

    const int topMenuMx = g_InputManager ? g_InputManager->GetMouseX() : 0;
    const int topMenuMy = g_InputManager ? g_InputManager->GetMouseY() : 0;
    const bool topMenuClick = g_InputManager && g_InputManager->IsMouseButtonPressed(0);
    ...
    for (size_t menuIndex = 0; menuIndex < topMenus.size(); ++menuIndex) {
        ...
        const bool hovered = topMenuMx >= menuX && topMenuMx <= menuX + menuWidth && topMenuMy >= 0 && topMenuMy <= 25;
        const uint32_t bgColor = isMenuActive ? 0xFF313131 : (hovered ? (topMenuClick ? 0xFF505050 : 0xFF3D3D3D) : 0x00000000);
        if ((bgColor >> 24) > 0) drawList.AddRectFilled(menuX, 0.0f, menuWidth, 25.0f, bgColor);
        drawList.AddText(fontMgr, topMenus[menuIndex], menuX + 15.0f, 20.5f, 0xFFE0E0E0);

        if (hovered && topMenuClick) {
    ...
    drawList.AddRectFilled(0, 25.0f, w, 40.0f, C_TOOLBAR);
    drawList.AddRectFilled(0, 64.0f, w, 1.0f, C_DIVIDER);
    ...
    drawList.AddRectFilled(rightX, topH, rightW, outlinerH, C_OUTLINER);
    drawList.AddRectFilled(rightX, topH, rightW, 30.0f, C_DETAILS);
    drawList.AddRectFilled(rightX, topH + 30.0f, rightW, 1.0f, C_DIVIDER);
    drawList.AddRectFilled(rightX, topH, 1.0f, outlinerH, C_BOTTOM);
    drawList.AddText(fontMgr, "Outliner", rightX + 15.0f, topH + 20.0f, C_TXTPANEL);
    ...
    if (hasSelectedMapAsset) {
        ...
        if (obj.type == ObjectType::PostProcessVolume) {
            ...
            uiCtx.DragFloat("Loc Z", obj.position.z, 0.05f, insX, insY, sW, 24.0f); insY += 28.0f;
            uiCtx.DragFloat("Scl X", obj.scale.x, 0.05f, insX, insY, sW, 24.0f); insY += 28.0f;
            uiCtx.DragFloat("Scl Y", obj.scale.y, 0.05f, insX, insY, sW, 24.0f); insY += 28.0f;
            uiCtx.DragFloat("Scl Z", obj.scale.z, 0.05f, insX, insY, sW, 24.0f); insY += 35.0f;

            drawList.AddRectFilled(insX, insY + 4.0f, sW, 1.0f, 0xFF383838);
            drawList.AddText(fontMgr, "Post Process", insX, insY + 15.0f, 0xFF9E9E9E);
            insY += 25.0f;
            uiCtx.DragFloat("Exposure", obj.ppSettings.exposure, 0.01f, insX, insY, sW, 24.0f); insY += 28.0f;
            uiCtx.DragFloat("Tint R", obj.ppSettings.colorTint.x, 0.01f, insX, insY, sW, 24.0f); insY += 28.0f;
            uiCtx.DragFloat("Tint G", obj.ppSettings.colorTint.y, 0.01f, insX, insY, sW, 24.0f); insY += 28.0f;
            uiCtx.DragFloat("Tint B", obj.ppSettings.colorTint.z, 0.01f, insX, insY, sW, 24.0f); insY += 35.0f;
            uiCtx.DragFloat("Bloom Thr", obj.ppSettings.bloomThreshold, 0.01f, insX, insY, sW, 24.0f); insY += 28.0f;
            uiCtx.DragFloat("Bloom Int", obj.ppSettings.bloomIntensity, 0.01f, insX, insY, sW, 24.0f); insY += 35.0f;
            uiCtx.DragFloat("Blend Rad", obj.ppSettings.blendRadius, 0.05f, insX, insY, sW, 24.0f); insY += 28.0f;
        } else if (obj.name.find("Sky") != std::string::npos) {
            drawList.AddRectFilled(insX, insY + 4.0f, sW, 1.0f, 0xFF383838);
    ...
    if (State.activeTopMenu >= 0) {
        ...
        case TopEditorMenu::Edit:
            menuItems = {
                ...
                {"Editor Preferences...", true},
                {"Project Settings...", !activeProjectFilePath.empty()},
            };
            break;
        case TopEditorMenu::Window:
            menuItems = {
                ...
                {std::string(renderer->GetEditorPreferences().showFpsCounter ? "* " : "  ") + "FPS Counter", true},
                {std::string(renderer->GetEditorPreferences().showRaytracingStats ? "* " : "  ") + "Raytracing Stats", true},
            };
            break;
        ...
        if (clickedMenuItem >= 0) {
            ...
            case TopEditorMenu::Edit:
                if (clickedMenuItem == 0 && hasSelectedBrowserItem) {
                    ...
                    OpenSettingsWindow(renderer, 1); // Editor > Viewport
                } else if (clickedMenuItem == 5 && !activeProjectFilePath.empty()) {
                    OpenSettingsWindow(renderer, 4); // Project > Rendering
                }
                break;
            case TopEditorMenu::Window:
                if (clickedMenuItem == 0) {
                    ...
                    EditorPreferences& viewPrefs = renderer->GetEditorPreferences();
                    viewPrefs.showFpsCounter = !viewPrefs.showFpsCounter;
                    renderer->SaveEditorPreferences();
                } else if (clickedMenuItem == 5) {
                    EditorPreferences& viewPrefs = renderer->GetEditorPreferences();
                    viewPrefs.showRaytracingStats = !viewPrefs.showRaytracingStats;
                    renderer->SaveEditorPreferences();
                }
                break;
            case TopEditorMenu::Tools:
    ...
    DrawViewportOverlay(renderer, topH, w - rightW);
